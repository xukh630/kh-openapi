...
import com.kh.openapi.common.constants.ErrorConstants;
...
public class ResultModle<T> implements Serializable {
     ...
     * -211 = 无效费率
     */
    public static <T> ResultModle<T> rateError(String rate){
        if (StringUtils.isBlank(rate)) {
            return new ResultModle(ErrorConstants.INVALID_RATE_CODE, ErrorConstants.INVALID_RATE_MSG);
        } else {
            return new ResultModle(ErrorConstants.INVALID_RATE_CODE, ErrorConstants.INVALID_RATE_MSG + ">>" + rate);
        }
    }
     ...
     * -224 = 配置不存在
     */
    public static <T> ResultModle<T> configError(){
        return new ResultModle(ErrorConstants.CONFIG_ERROR_CODE, ErrorConstants.CONFIG_ERROR_MSG);
    }
